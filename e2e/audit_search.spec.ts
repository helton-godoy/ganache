...
test.describe("Audit Search E2E", () => {
  ...
  test("should search by filename and display results with User, IP, and Timestamp", async ({
    page,
  }) => {
  ...
  test("should show export buttons when results exist (AC1: PDF/CSV export)", async ({
    page,
  }) => {
    await page.goto("/audit");
  ...
  test("should show empty state when no results found", async ({ page }) => {
    ...
    await expect(
      page.getByText(/No events found for "nonexistent.txt"/i),
    ).toBeVisible();
    await expect(
