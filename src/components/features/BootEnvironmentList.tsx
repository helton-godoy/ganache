...
import { useGetBootEnvironments, useActivateBootEnvironment } from "@/api/generated/default/default";
    ...
    const activateMutation = useActivateBootEnvironment({
        mutation: {
            onSuccess: (axiosRes) => {
                ...
                void queryClient.invalidateQueries({ queryKey: getGetBootEnvironmentsQueryKey() });
            },
            onError: (err: any) => {
    ...
    return (
        <DialogContent className="sm:max-w-3xl">
            <DialogHeader>
                ...
                <DialogDescription>
                    Manage ZFS Boot Environments. The active environment will be used on the next system boot.
                </DialogDescription>
            </DialogHeader>
                            ...
                            return (
                                <TableRow key={be.name}>
                                    <TableCell className="font-medium">
                                        {be.name}
                                        {isNow && <span className="ml-2 text-xs text-muted-foreground">(Current)</span>}
                                    </TableCell>
