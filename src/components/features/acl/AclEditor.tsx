...
export function AclEditor({ path }: AclEditorProps) {
    ...
    const handleSaveAce = (ace: Nfs4Ace) => {
        ...
        if (ace.permissions.write_acl || ace.permissions.write_owner) {
            toast.warning(`Warning: ACE grants sensitive permissions (${ace.permissions.write_acl ? 'write_acl' : 'write_owner'})`);
        }
                            ...
                            {aces.length === 0 ? (
                                ...
                                aces.map((ace, index) => (
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>
                                            <Badge variant={ace.ace_type === AceType.allow ? "default" : "destructive"}>
                                                {ace.ace_type}
                                            </Badge>
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <div className="flex items-center space-x-2">
                                                <Button variant="ghost" size="icon" onClick={() => handleEditAce(index)}>
                                                    <span className="sr-only">Edit</span>
                                                    <Users className="h-4 w-4" /> {/* Placeholder icon */}
                                                </Button>
                                                <Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleDeleteAce(index)}>
...
function PrincipalDisplay({ principal }: { principal: AcePrincipal }) {
    ...
    if (principal === "everyone") return <Badge variant="outline">EVERYONE@</Badge>;

    if (typeof principal === 'object') {
        if ('user' in principal) {
            return <div className="flex items-center"><User className="mr-1 h-3 w-3" /> {principal.user}</div>;
        }
        if ('named_group' in principal) {
            return <div className="flex items-center"><Users className="mr-1 h-3 w-3" /> {principal.named_group}</div>;
        }
    }
...
function PermissionsDisplay({ permissions }: { permissions: Nfs4Permissions }) {
    // Simple summary
    if (permissions.read_data && permissions.write_data) return <span>Read/Write</span>;
...
function AceDialog({ open, onOpenChange, ace, onSave }: AceDialogProps) {
    ...
    const [principalType, setPrincipalType] = useState<"user" | "group" | "special">("special");
    ...
    useEffect(() => {
        if (ace) {
            ...
            if (ace.principal === "owner" || ace.principal === "group" || ace.principal === "everyone") {
                ...
                <div className="grid gap-4 py-4">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3">
                            <Select value={principalType} onValueChange={(v: "user" | "group" | "special") => setPrincipalType(v)}>
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                    ...
                    {principalType === "special" && (
                        <div className="grid grid-cols-4 items-center gap-4">
                            ...
                            <div className="col-span-3">
                                <Select value={specialPrincipal} onValueChange={setSpecialPrincipal}>
                                    <SelectTrigger>
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                        ...
                                        <SelectItem value="everyone">Everyone (everyone@)</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3">
                            <Select value={localAce.ace_type} onValueChange={(v) => setLocalAce({ ...localAce, ace_type: v as AceType })}>
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                    ...
                    <div className="grid grid-cols-4 items-start gap-4">
                        ...
                        <div className="col-span-3 grid grid-cols-2 gap-2">
                            <PermissionCheckbox
                                ...
                                onChange={(c) => setLocalAce({ ...localAce, permissions: { ...localAce.permissions, read_data: c } })}
                            />
                            <PermissionCheckbox
                                label="Write Data"
                                checked={localAce.permissions.write_data}
                                onChange={(c) => setLocalAce({ ...localAce, permissions: { ...localAce.permissions, write_data: c } })}
                            />
                            <PermissionCheckbox
                                label="Execute"
                                checked={localAce.permissions.execute}
                                onChange={(c) => setLocalAce({ ...localAce, permissions: { ...localAce.permissions, execute: c } })}
                            />
                            <PermissionCheckbox
                                label="Delete"
                                checked={localAce.permissions.delete}
                                onChange={(c) => setLocalAce({ ...localAce, permissions: { ...localAce.permissions, delete: c } })}
                            />
                            <PermissionCheckbox
                                label="Append"
                                checked={localAce.permissions.append_data}
                                onChange={(c) => setLocalAce({ ...localAce, permissions: { ...localAce.permissions, append_data: c } })}
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-4 items-start gap-4">
                        ...
                        <div className="col-span-3 grid grid-cols-2 gap-2">
                            <PermissionCheckbox
                                ...
                                onChange={(c) => setLocalAce({ ...localAce, inherit_flags: { ...localAce.inherit_flags, file_inherit: c } })}
                            />
                            <PermissionCheckbox
                                label="Dir Inherit"
                                checked={localAce.inherit_flags.dir_inherit}
                                onChange={(c) => setLocalAce({ ...localAce, inherit_flags: { ...localAce.inherit_flags, dir_inherit: c } })}
                            />
                        </div>
                    </div>
...
function PrincipalSearch({ type, value, onChange }: { type: "user" | "group", value: string, onChange: (v: string) => void }) {
