        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
                <div className="bg-destructive/10 border-destructive border p-6 rounded-xl animate-pulse">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-destructive">System Panic: CRITICAL</h2>
                            <p className="text-destructive/80 font-medium">Master node unreachable. Split-brain detected.</p>
                ...
                <div className="bg-card border rounded-xl p-6 shadow-sm">
                    ...
                    <div className="space-y-4">
                        <div className="p-4 border rounded-lg bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer"
                            onClick={() => setSelectedNode('node-02')}
                        >
                            <div className="flex items-center justify-between mb-2">
                                <span className="font-bold">Node-02 (Standby)</span>
                                <span className="px-2 py-0.5 text-xs bg-emerald-100 text-emerald-700 rounded-full font-medium">Healthy</span>
                            </div>
                            <p className="text-sm text-muted-foreground mb-3">Last synced: 2s ago. Ready for promotion.</p>
                            <Button
                        ...
                        >
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="px-2 py-0.5 text-xs bg-yellow-100 text-yellow-700 rounded-full font-medium">Degraded</span>
                            </div>
                            <p className="text-sm text-muted-foreground mb-3">High latency detected. Promotion not recommended.</p>
                            <Button
                                disabled={promoteMutation.isPending}
                                onClick={(e) => {
                                    e.stopPropagation();
            ...
            <div className="bg-card border rounded-xl overflow-hidden shadow-sm flex flex-col max-h-[600px]">
                ...
                <div className="flex-1 overflow-y-auto p-0">
                    {logs?.map((log) => (
                        <div key={log.id} className="p-4 border-b last:border-0 hover:bg-muted/5">
                            <div className="flex items-center gap-2 mb-1">
                                <span className={cn(
                                    "text-xs font-bold px-1.5 py-0.5 rounded",
                                    log.level === 'CRITICAL' && "bg-destructive text-destructive-foreground",
