    ...
    return (
        <div className="bg-white rounded-lg shadow overflow-hidden flex flex-col h-[600px]">
            <div className="p-4 border-b border-gray-200 bg-gray-50">
                <h3 className="text-lg font-semibold text-gray-800">Security Event Feed</h3>
                            ...
                            <div className="flex justify-between items-start">
                                ...
                                {(event.source_ip || event.user) && (
                                    <div className="text-right text-xs opacity-80">
                                        {event.user && <div>User: <span className="font-mono">{event.user}</span></div>}
                                        {event.source_ip && <div>IP: <span className="font-mono">{event.source_ip}</span></div>}
                                    </div>
                                )}
                            </div>
                        </div>
