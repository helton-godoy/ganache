...
export function ClusterConnectionVisualizer({
  status,
}: ClusterConnectionVisualizerProps) {
  ...
  const isSyncing =
    status.state === "syncing" || status.state === "configuring";
      ...
      <div className="flex items-center gap-12">
        ...
        <div className="relative flex flex-col items-center gap-2 w-32">
          <div className="w-full h-1 bg-slate-700 overflow-hidden rounded-full">
            {isSyncing && (
              <div
                className="h-full bg-blue-500 w-1/2 animate-[shimmer_1s_infinite]"
                style={{
        ...
        <div className="flex flex-col items-center gap-2">
          <Server
            className={`w - 12 h - 12 ${isReady ? "text-emerald-500" : "text-slate-500"} `}
          />
