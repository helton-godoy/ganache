...
export function WizardCompatibilityStep({
  onNext,
  onBack,
}: WizardCompatibilityStepProps) {
  ...
  if (isPending || (status && status.data)) {
    const clusterStatus = status?.data as any as {
      state: "configuring" | "syncing" | "ready" | "error";
      progress: number;
      message: string;
    };
    return (
      <ClusterConnectionVisualizer
        status={
          clusterStatus || {
            state: "configuring",
            progress: 0,
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Understanding how Ganache ensures reliability on top of your existing
          hardware RAID.
        </p>
      </div>
        ...
        <div className="relative w-full max-w-sm space-y-2 p-6 bg-slate-50/50 rounded-xl border-2 border-dashed border-slate-200">
            ...
            <Tooltip delayDuration={300}>
              ...
              <TooltipContent>
                <p className="max-w-xs">
                  ZFS handles data checksums, compression, and snapshots. It
                  thinks it is writing to a whole disk, but is actually writing
          ...
          <TooltipProvider>
            <Tooltip delayDuration={300}>
              ...
              <TooltipContent>
                <p className="max-w-xs">
                  DRBD mirrors every write to the secondary node over the
                  network before confirming success to ZFS.
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <TooltipProvider>
            <Tooltip delayDuration={300}>
              ...
              <TooltipContent>
                <p className="max-w-xs">
                  Your physical PERC/RAID controller handles disk failures
                  locally. Ganache sees this as a single reliable "Virtual
                  Drive".
                </p>
              </TooltipContent>
            </Tooltip>
          ...
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                This "Layer Cake" approach allows ZFS to run safely without
                needing direct access to disks, relying on the hardware
                controller for physical redundancy.
              </p>
            </div>
          </div>
        </div>
      ...
      <ConfirmationDialog
        ...
        description={
          <div className="space-y-2">
            <p>
              You are about to configure a Twin-View Cluster using{" "}
              <strong>Compatibility Mode</strong>.
            </p>
            <p className="text-sm text-muted-foreground">
              This involves layering ZFS on top of your hardware RAID Virtual
              Drives via DRBD network replication.
            </p>
