...
export function SetupWizard({
  ...
  useEffect(() => {
    if (
      serverDisks &&
      nodeAAvailable.length === 0 &&
      nodeAAssigned.length === 0 &&
      nodeBAvailable.length === 0 &&
      nodeBAssigned.length === 0
    ) {
  ...
  const handleAutoAssignNodeA = () => {
    setNodeAAssigned(
      [...nodeAAvailable, ...nodeAAssigned].map((d) => ({
        ...d,
        status: "assigned",
      })),
    );
  ...
  const handleAutoAssignNodeB = () => {
    setNodeBAssigned(
      [...nodeBAvailable, ...nodeBAssigned].map((d) => ({
        ...d,
        status: "assigned",
      })),
    );
    ...
    <div className="space-y-6 max-w-4xl mx-auto p-6 bg-card rounded-xl border shadow-sm">
      <div className="text-center space-y-2 mb-8">
        ...
        <p className="text-muted-foreground">
          Please verify the configuration before building the cluster.
        </p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-muted/30 p-4 rounded-lg space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Pool Disks:</span>
              <span className="font-mono font-bold">
                {nodeBAssigned.length}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Total Raw Capacity:</span>
              <span className="font-mono">
                ~{(nodeBAssigned.length * 1.92).toFixed(2)} TB
              </span>
            </div>
            <ul className="text-xs text-muted-foreground list-disc pl-4 pt-2">
              {nodeBAssigned.map((d) => (
            ...
            <div className="flex flex-col md:flex-row gap-4 text-sm text-muted-foreground">
              ...
              {currentStep === "review" && (
                <span className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-green-500" /> Ready to apply
                </span>
              )}
            </div>
