...
export function DatasetManager({ poolName }: DatasetManagerProps) {
  const {
    ...
    isLoading,
    refetch,
  } = useListDatasets({ pool: poolName });
  const datasets = datasetsResponse?.data;
  const { mutate: destroyMutate, isPending: isDestroyPending } =
    useDestroyDataset();
  ...
  const [aclDataset, setAclDataset] = useState<{
    name: string;
    mountpoint: string;
  } | null>(null);
    ...
    <Card className="mt-8">
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Manage ZFS child datasets for {poolName}
          </CardDescription>
        </div>
      ...
      <Dialog
        open={!!aclDataset}
        onOpenChange={(open) => !open && setAclDataset(null)}
      >
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Manage Permissions</DialogTitle>
            <DialogDescription>
              Configure NFSv4 ACLs for dataset{" "}
              <strong>{aclDataset?.name}</strong>
              <br />
              Mountpoint:{" "}
              <code className="text-xs">{aclDataset?.mountpoint}</code>
            </DialogDescription>
          </DialogHeader>
          {aclDataset && (
            <div className="py-4">
