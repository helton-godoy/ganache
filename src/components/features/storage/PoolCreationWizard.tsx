...
export function PoolCreationWizard() {
    ...
    mutation: {
      onSuccess: () => {
        void queryClient.invalidateQueries({
          queryKey: getGetSystemResourcesQueryKey(),
        });
        setOpen(false);
        setName("");
        setSelectedDevice("");
  ...
  const isReady =
    name.length >= 3 && selectedDevice && confirmation === "CONFIRM";
      ...
      <DialogContent className="sm:max-w-[450px] bg-slate-900 border-slate-800 text-slate-100 backdrop-blur-xl shadow-2xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-slate-400 text-center">
            Crie um pool ZFS sobre um dispositivo replicado DRBD para garantir
            Resiliência HA.
          </DialogDescription>
        </DialogHeader>
              ...
              {drbdDevices.length === 0 && (
                <div className="p-8 text-center border border-dashed border-slate-800 rounded-xl bg-slate-950/20">
                  ...
                  <p className="text-sm text-slate-500">
                    Nenhum dispositivo DRBD encontrado.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="mt-2 p-4 bg-yellow-500/5 border border-yellow-500/20 rounded-xl space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  Esta operação formatará o dispositivo selecionado e criará a
                  estrutura ZFS. Todos os dados existentes no dispositivo DRBD
                  serão perdidos.
                </p>
              </div>
            </div>
            <div className="space-y-2">
              <p className="text-[10px] text-slate-500 font-medium ml-1">
                Digite{" "}
                <span className="text-yellow-500/80 font-mono italic">
                  CONFIRM
                </span>{" "}
                para validar:
              </p>
        ...
        <DialogFooter className="pt-2">
          <Button
            ...
            className={`w-full h-12 text-sm font-bold tracking-wide transition-all duration-300 ${
              isReady
