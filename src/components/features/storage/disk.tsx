...
export function DiskCard({
  ...
  isDragging,
  isOverlay,
}: DiskProps & { isDragging?: boolean; isOverlay?: boolean }) {
  return (
    <div
      onDoubleClick={(e) => {
        if (onDoubleClick) {
      ...
      className={cn(
        ...
        disk.status === "assigned" &&
          "cursor-default opacity-100 pr-10 hover:border-destructive/50",
        ...
        isOverlay &&
          "z-50 shadow-xl scale-105 ring-2 ring-primary ring-offset-2 opacity-100 bg-card",
      )}
