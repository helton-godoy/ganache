...
test.describe('Active Directory Integration', () => {
    ...
    test('should display AD status endpoint', async ({ request }) => {
        const response = await request.get('http://localhost:3005/api/v1/ad/status');
    ...
    test('should validate required fields on join request', async ({ request }) => {
        ...
        const response = await request.post('http://localhost:3005/api/v1/ad/join', {
            data: invalidRequest,
            headers: {
    ...
    test('should allow leaving domain', async ({ request }) => {
        const response = await request.post('http://localhost:3005/api/v1/ad/leave', {
            headers: {
                'X-Auth-User': 'testuser'
            }
        });
    ...
    test('should persist AD configuration to Git', async ({ request }) => {
        ...
        const historyResponse = await request.get('http://localhost:3005/api/v1/config/history?limit=10');
        ...
        const adCommit = commits.find((commit: any) =>
    ...
    test('should update status after join', async ({ request }) => {
        ...
        const statusBefore = await request.get('http://localhost:3005/api/v1/ad/status');
        ...
        const statusAfter = await request.get('http://localhost:3005/api/v1/ad/status');
