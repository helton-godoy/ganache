...
test.describe('Break-Glass Emergency Admin - Activation [P0]', () => {
    ...
    test('[P1] should record activation metadata (who, when, why, source IP)', async ({
        ...
        const response = await request.post('/api/v1/security/break-glass/activate', {
            data: {
                reason,
    ...
    test('[P1] should prevent duplicate activation (idempotency)', async ({ request }) => {
        // WHEN: Activating break-glass
        const firstResponse = await request.post('/api/v1/security/break-glass/activate', {
            data: {
        ...
        const secondResponse = await request.post('/api/v1/security/break-glass/activate', {
            data: {
