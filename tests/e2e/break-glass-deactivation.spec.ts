...
test.describe('Break-Glass Emergency Admin - Deactivation [P1]', () => {
    test('[P1] should deactivate account when AD is restored', async ({
        ...
        await expect(
            breakGlassActivated.locator('[data-testid="confirm-deactivation-dialog"]')
        ).toBeVisible();

        // WHEN: Admin confirms deactivation
        await breakGlassActivated.click('[data-testid="confirm-deactivation-button"]');

        // THEN: Success message is displayed
        await expect(
            breakGlassActivated.locator('[data-testid="deactivation-success-message"]')
        ).toContainText(/deactivated successfully|disabled/i);

        // AND: Status page shows "Disabled"
        const statusResponse = await request.get('/api/v1/security/break-glass/status');
    ...
    test('[P1] should generate security event on deactivation', async ({
        ...
        const firstEvent = breakGlassActivated.locator('[data-testid="security-event"]').first();
        await expect(firstEvent).toContainText(/BreakGlassDeactivation|Deactivated/i);
        await expect(firstEvent).toContainText(/High|Critical/i); // Severity
    });

    test('[P1] should clear activation info after deactivation', async ({ request }) => {
        ...
        const beforeStatus = await request.get('/api/v1/security/break-glass/status');
        const beforeBody = await beforeStatus.json();
        expect(beforeBody.activation_info).toBeDefined();
        expect(beforeBody.activation_info.reason).toBe('Test activation for deactivation');

        // WHEN: Deactivating
        await request.post('/api/v1/security/break-glass/deactivate');

        // THEN: Activation info is cleared
        const afterStatus = await request.get('/api/v1/security/break-glass/status');
    ...
    test('[P2] should notify administrators on deactivation (if implemented)', async ({
        ...
        const response = await request.post('/api/v1/security/break-glass/deactivate', {
            data: {
    ...
    test('[P2] should prevent operations on disabled account', async ({ request }) => {
...
test.describe('Break-Glass Emergency Admin - Audit Report [P2]', () => {
    test('[P2] should track session duration in deactivation event', async ({ request }) => {
        ...
        expect(deactivationEvent.details.duration_seconds).toBeGreaterThanOrEqual(2);
    });
});
