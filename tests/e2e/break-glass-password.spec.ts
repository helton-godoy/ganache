...
test.describe('Break-Glass Emergency Admin - Password Security [P0]', () => {
    test('[P0] should force password reset before first login', async ({
        ...
        await breakGlassActivated.fill('[data-testid="username"]', 'emergency_admin');
        ...
        await expect(breakGlassActivated).toHaveURL(/\/reset-password/, { timeout: 10000 });
        ...
        await expect(
            breakGlassActivated.locator('[data-testid="password-reset-required-message"]')
    ...
    test('[P0] should enforce password complexity (12+ chars, mixed case, numbers, symbols)', async ({
        ...
        await breakGlassActivated.fill('[data-testid="new-password"]', tooShortPassword);
        ...
        await breakGlassActivated.fill('[data-testid="new-password"]', noUpperPassword);
        await breakGlassActivated.click('[data-testid="submit-password"]');
        ...
        await breakGlassActivated.fill('[data-testid="new-password"]', noLowerPassword);
        await breakGlassActivated.click('[data-testid="submit-password"]');
        ...
        await breakGlassActivated.fill('[data-testid="new-password"]', noDigitPassword);
        await breakGlassActivated.click('[data-testid="submit-password"]');
        ...
        await breakGlassActivated.fill('[data-testid="new-password"]', noSymbolPassword);
        await breakGlassActivated.click('[data-testid="submit-password"]');
        ...
        await breakGlassActivated.fill('[data-testid="new-password"]', validPassword);
        await breakGlassActivated.fill('[data-testid="confirm-password"]', validPassword);
        await breakGlassActivated.click('[data-testid="submit-password"]');
    ...
    test('[P1] should validate password via API endpoint', async ({ request }) => {
        ...
        expect(invalidBody.errors).toContain('Password must be at least 12 characters');
    ...
    test('[P2] should log password reset activity to audit log', async ({
        ...
        await breakGlassActivated.fill('[data-testid="confirm-password"]', newPassword);
        await breakGlassActivated.click('[data-testid="submit-password"]');
        ...
        await breakGlassActivated.waitForSelector('[data-testid="security-event"]', {
            timeout: 5000,
        });

        // Search for password reset event
        const events = breakGlassActivated.locator('[data-testid="security-event"]');
        const eventCount = await events.count();

        let foundPasswordResetEvent = false;
        for (let i = 0; i < eventCount; i++) {
            const eventText = await events.nth(i).textContent();
            if (eventText?.includes('password') || eventText?.includes('PasswordReset')) {
