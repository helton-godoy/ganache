...
test.describe("Hardware Detection", () => {
  test("should show standard mode by default when no raid detected (mocked)", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Standard Mode" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Compatibility Mode" }),
    ).toBeVisible();
  });
