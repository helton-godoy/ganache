...
test.describe('Configuration Rollback', () => {
    ...
    test('should open confirmation modal when rollback is clicked', async ({ page }) => {
        ...
        await expect(page.locator('text=You are about to rollback the configuration to commit')).toBeVisible();
        await expect(page.getByPlaceholder('e.g., Bad network configuration causing connectivity issues')).toBeVisible();
    });
    ...
    test('should allow canceling rollback', async ({ page }) => {
        ...
        await expect(page.locator('text=Confirm Configuration Rollback')).not.toBeVisible();
    });

    test('should enable confirm button when reason is provided', async ({ page }) => {
        // Click rollback button
        const rollbackButton = page.locator('button:has-text("Rollback to this Point")').first();
        await rollbackButton.click();

        // Fill in the reason
        const reasonInput = page.getByPlaceholder('e.g., Bad network configuration causing connectivity issues');
    ...
    test('should successfully execute rollback with valid reason', async ({ page }) => {
    ...
    test('should show loading state during rollback execution', async ({ page }) => {
        // Click rollback button
        const rollbackButton = page.locator('button:has-text("Rollback to this Point")').first();
        await rollbackButton.click();
