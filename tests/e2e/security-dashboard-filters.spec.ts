...
test.describe("Security Dashboard - Filters", () => {
  ...
  test("[P0] should filter events by type (SSH, Config, Auth)", async ({
    authenticatedPage,
  }) => {
    // GIVEN: Dashboard is loaded with multiple event types
    const allEvents = authenticatedPage.locator(
    ...
    const filteredEvents = authenticatedPage.locator(
      '[data-testid="security-event"]',
    );
  ...
  test("[P1] should filter events by user", async ({ authenticatedPage }) => {
    ...
    const firstUserOption = authenticatedPage
      .locator('[data-testid^="filter-user-option-"]')
      .first();
    ...
    await expect(
      authenticatedPage.locator(
        `[data-testid="active-filter-user-${userName}"]`,
      ),
    ).toBeVisible();
  });

  test("[P1] should filter events by IP address", async ({
    authenticatedPage,
  }) => {
    // GIVEN: Dashboard with events from multiple IPs
    const ipFilterInput = authenticatedPage.locator(
      '[data-testid="filter-ip-input"]',
    );
    ...
    if (count > 0) {
      ...
      await expect(
        authenticatedPage.locator('[data-testid="no-events-message"]'),
      ).toBeVisible();
    }
  });
  ...
  test("[P2] should filter events by time period (last hour, last 24h, custom)", async ({
    ...
    const timeFilter = authenticatedPage.locator(
    ...
    for (let i = 0; i < Math.min(count, 3); i++) {
      const timestamp = await filteredEvents
        .nth(i)
  ...
  test("[P1] should combine multiple filters (type + user)", async ({
    authenticatedPage,
  }) => {
    ...
    const firstUser = authenticatedPage
      .locator('[data-testid^="filter-user-option-"]')
      .first();
