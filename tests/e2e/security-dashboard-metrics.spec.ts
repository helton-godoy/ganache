...
test.describe('Security Dashboard - Metrics', () => {
    test.beforeEach(async ({ authenticatedPage }) => {
        ...
        await authenticatedPage.waitForSelector('[data-testid="metric-events-min"]', {
            timeout: 10000,
        });
    });
    ...
    test('[P1] should calculate and display events per minute correctly', async ({
        ...
        await request.post('/api/v1/security/events', {
            data: { events: [event1, event2, event3] },
        }).catch(() => {
            // If batch not supported, may need to inject individually or skip
            console.warn('Event injection not supported - test may validate existing data only');
        });
    ...
    test('[P1] should display active users count', async ({ authenticatedPage }) => {
        // GIVEN: Dashboard is loaded
        const activeUsersMetric = authenticatedPage.locator('[data-testid="metric-active-users"]');
    ...
    test('[P2] should identify and display suspicious IPs', async ({ authenticatedPage }) => {
        // GIVEN: Dashboard is loaded
        ...
        const ipList = authenticatedPage.locator('[data-testid="suspicious-ip-item"]');
    ...
    test('[P1] should update metrics in real-time when new events arrive', async ({
        ...
        const eventsMetric = authenticatedPage.locator('[data-testid="metric-events-min"]');
    ...
    test('[P2] should display metric trends (increasing/decreasing indicators)', async ({
        ...
        const trendIndicator = authenticatedPage.locator('[data-testid="metric-events-trend"]');
