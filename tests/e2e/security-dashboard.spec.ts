...
test.describe("Security Dashboard", () => {
  test("[P1] should display key elements and real backend data", async ({
    authenticatedPage,
  }) => {
    ...
    const connectionStatus = authenticatedPage
      .locator("text=Live Connected, text=Disconnected")
      .first();
    await expect(connectionStatus).toBeVisible({ timeout: 5000 });

    // Verify Event Timeline Component
    await expect(
      authenticatedPage.getByText("Security Event Feed"),
    ).toBeVisible();
