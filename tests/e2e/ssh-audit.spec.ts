...
test.describe("Compliance: Deep SSH Audit Logging (Story 5.1)", () => {
   ...
   */
  test("should record standard shell commands in the security feed", async ({
    page,
  }) => {
    ...
    const eventRow = page.locator(
      '[data-testid="event-timeline"] >> text=' + testCommand,
    );
   ...
   */
  test("should capture commands executed within sub-shells", async ({
    page,
  }) => {
    ...
    const criticalLog = page.locator(
      '[data-testid="event-timeline"] >> text=' + nestedCommand,
    );

    // THEN: The nested command must be identified and displayed
    await expect(criticalLog).toBeVisible();
    await expect(criticalLog).toHaveAttribute(
      "data-security-level",
      "CRITICAL",
    );
  });
