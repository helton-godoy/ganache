...
test.describe("Wizard Mode Selection", () => {
  test("should recommend Compatibility Mode when hardware RAID is detected", async ({
    wizard,
    page,
  }) => {
    // GIVEN: Hardware RAID is detected
    await wizard.mockHardwareDetection({
      has_raid: true,
      controller_name: "PERC H700",
    });
  ...
  test("should show safety gate warning when selecting Standard Mode with RAID present", async ({
    wizard,
    page,
  }) => {
    // GIVEN: Hardware RAID is detected
