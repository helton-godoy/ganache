...
export const mockSetupStatus = (step = 1) => ({
    ...
    totalSteps: 4,
    steps: [
        { id: 1, name: 'Hardware Detection', status: step > 1 ? 'completed' : 'active' },
        { id: 2, name: 'Network Configuration', status: step > 2 ? 'completed' : step === 2 ? 'active' : 'pending' },
        { id: 3, name: 'Storage Setup', status: step > 3 ? 'completed' : step === 3 ? 'active' : 'pending' },
        { id: 4, name: 'Cluster Initialization', status: step === 4 ? 'active' : 'pending' },
    ],
});
