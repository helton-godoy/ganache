...
test.describe('SSR Error Messages & Deploy Blocker (@p0 @ssr)', () => {
     ...
     */
    test('should render security dashboard with audit logs via SSR @p0 @6.4-E2E-004', async ({ page }) => {
     ...
     */
    test('should detect and handle client-only API usage in SSR @p0 @6.4-E2E-005', async ({ page }) => {
        ...
        const hasClientOnlyError = consoleErrors.some(err =>
        ...
        expect(hasClientOnlyError, 'SSR should not have client-only API errors').toBe(false);
     ...
     */
    test('should not expose sensitive data in initial SSR HTML @p0 @6.4-E2E-006 @security', async ({ page }) => {
        ...
        expect(foundSensitiveData.length, 'Sensitive data should NOT be in SSR HTML').toBe(0);
     ...
     * AC #1: "Mensagens de erro claras"
     */
    test('should catch and display errors from Server Components @p0 @6.4-INT-001', async ({ page }) => {
     ...
     */
    test('should fail CI/CD pipeline if critical SSR tests fail @p0 @6.4-E2E-007 @ci', async ({ page }) => {
