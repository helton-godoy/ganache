...
test.describe("SSR Failure Detection (@p0 @ssr @smoke)", () => {
   ...
   */
  test("should render root layout without SSR errors @p0 @6.4-E2E-001", async ({
    page,
  }) => {
    ...
    const hasSSRError = consoleErrors.some(
      (err) =>
    ...
    expect(
      hasSSRError,
      `SSR errors detected: ${consoleErrors.join(", ")}`,
    ).toBe(false);
   ...
   */
  test("should render dashboard with initial SSR data @p0 @6.4-E2E-002", async ({
    page,
  }) => {
   ...
   */
  test("should render setup wizard complete flow via SSR @p0 @6.4-E2E-003", async ({
    page,
  }) => {
    ...
    expect(
      consoleLogs.length,
      `Hydration mismatches: ${consoleLogs.join(", ")}`,
    ).toBe(0);
  });
});
