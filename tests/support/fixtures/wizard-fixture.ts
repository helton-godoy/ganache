...
export class WizardFixture {
  ...
  async mockHardwareDetection(
    hardwareInfo: { has_raid: boolean; controller_name: string | null } = {
      has_raid: false,
  ...
  async goto({ skipWelcome = true }: { skipWelcome?: boolean } = {}) {
    ...
    if (skipWelcome) {
      ...
      await expect(
        this.page.getByText("Twin-View Cluster Setup"),
      ).toBeVisible();
      // Wait for data to load to prevent race condition on Auto-Fill
      await expect(
        this.page.getByRole("button", { name: /Auto-Fill/i }).first(),
      ).toBeVisible();
    } else {
  ...
  async proceedToReview() {
    ...
    await expect(this.page.getByText("Pragmatic Mode"))
